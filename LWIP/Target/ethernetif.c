...
/* USER CODE BEGIN 4 */
static void ethernet_make_mac_address(uint8_t *mac)
{
  const uint32_t uid0 = HAL_GetUIDw0();
  const uint32_t uid1 = HAL_GetUIDw1();
  const uint32_t uid2 = HAL_GetUIDw2();

  /* Locally administered unicast MAC derived from the MCU's unique ID. */
  mac[0] = 0x02U;
  mac[1] = (uint8_t)(uid0 >> 0);
  mac[2] = (uint8_t)(uid0 >> 8);
  mac[3] = (uint8_t)(uid0 >> 16);
  mac[4] = (uint8_t)(uid1 ^ uid2);
